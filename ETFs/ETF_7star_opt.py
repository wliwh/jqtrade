...
class Config:
    ...
    # 动量计算
    LOOKBACK_DAYS = 25
    MIN_SCORE_THRESHOLD = EXECUTION_SCORE_THRESHOLD[0]
    MAX_SCORE_THRESHOLD = EXECUTION_SCORE_THRESHOLD[1]

    # 过滤器配置
    USE_SHORT_MOMENTUM_FILTER = EXECUTION_SHORT_MOMENTUM_PARAM[0]
    ...
    MIN_ANNUAL_RETURN = EXECUTION_ANNUAL_RETURN_PARAM[1]
    ...
    ENABLE_LOSS_FILTER = EXECUTION_LOSS_PARAM[0]
    LOSS = EXECUTION_LOSS_PARAM[1]

    USE_RSI_FILTER = EXECUTION_RSI_PARAM[0]
    RSI_PERIOD = EXECUTION_RSI_PARAM[1]
    RSI_LOOKBACK_DAYS = EXECUTION_RSI_PARAM[2]
    RSI_THRESHOLD = EXECUTION_RSI_PARAM[3]

    # 止损配置
    USE_FIXED_STOP_LOSS = EXECUTION_FIXED_STOPLOSS[0]
    FIXED_STOP_LOSS_THRESHOLD = EXECUTION_FIXED_STOPLOSS[1]
    USE_PCT_STOP_LOSS = EXECUTION_PCT_STOPLOSS[0]
    PCT_STOP_LOSS_THRESHOLD = EXECUTION_PCT_STOPLOSS[1]
    USE_ATR_STOP_LOSS = EXECUTION_ATR_STOPLOSS[0]
    ATR_PERIOD = EXECUTION_ATR_STOPLOSS[1]
    ATR_MULTIPLIER = EXECUTION_ATR_STOPLOSS[2]
    ATR_TRAILING_STOP = EXECUTION_ATR_STOPLOSS[3]
    ATR_EXCLUDE_DEFENSIVE = EXECUTION_ATR_STOPLOSS[4]

    # 冷却期
    SELL_COOLDOWN_ENABLED = EXECUTION_SELL_COOLDOWN[0]
    SELL_COOLDOWN_DAYS = EXECUTION_SELL_COOLDOWN[1]
