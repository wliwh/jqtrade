...
1.  **第一阶段 (引入 Filter)**：保持 `Slope * R^2` 排序逻辑不变。引入 `Reflex` 作为**卖出信号**（辅助风控），引入 `Fractal Dimension` 作为**准入过滤器**（$D > 1.6$ 的不买）。
2.  **第二阶段 (替换 R2)**：使用 `Slope * (2 - D)` 进行排序，测试是否优于 `Slope * R^2`。
3.  **第三阶段 (全面升级)**：使用 `Trendflex * (2 - D)`，完全替换掉线性回归体系，实现低滞后轮动。