...
*   DBSCAN (Density-Based Spatial Clustering of Applications with Noise)：基于密度的聚类方法。它不需要预先指定簇的数量。算法将紧密相连的高密度区域划分为簇，而处于低密度区域的点会被标记为“噪声” (Noise)。这种方法的一大优势是能够自动识别并保留那些独特的、不随大流的“独行侠”资产 (Outliers)，有效地增加了池子的多样性。
