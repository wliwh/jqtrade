...
- Code 字段非空：优先使用 `traced_index_code` 作为合并的唯一键值。
- Code 为空但 Name 非空：退而求其次使用 `traced_index_name` 作为键值。*注：考虑到可能存在名称略微差异但底层指数相同的情况，这部分逻辑未来仍有精进空间。*
- 两者均为空：目前观察到仅出现在商品黄金类的 ETF 中，对此类特殊标的采用硬编码方式进行兜底处理。*后期可优化*
...
本次更新属于功能性的“小补丁”，相关的修改代码已整理并存放在附件中。大家无需重新搭建研究环境，只需按照说明替换原有文件即可。希望这次微调能为大家的策略研究带来帮助，也欢迎各位交流反馈，点赞支持！