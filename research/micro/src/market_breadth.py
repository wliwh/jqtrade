...
ARTIFACT_DATA_DIR = Path(__file__).resolve().parents[1] / "artifacts" / "data"
...
def get_csv_last_date():
    ...
    """
    try:
        csv_path = ARTIFACT_DATA_DIR / CONFIG['csv_filename']
...
def save_to_csv(df_new):
    ...
    """
    # 所有可再生研究数据统一保存到 research/micro/artifacts/data/。
    try:
        save_dir = ARTIFACT_DATA_DIR
