...
from .regions import build_turning_point_regions
...
def write_viewer(
    ...
    for position, (index_id, index_name, _, relative_path, float_prices) in enumerate(INDEX_SPECS):
        ...
        chart_specs.append(f'"{chart_id}":{pio.to_json(figure)}')
        active = position == 0
        tabs.append(
...
<head>
  ...
  <title>指数顶底区域检查 · 基础尺度 {threshold:.0%}</title>
  <style>
    :root {{
...
<body>
  <header class="topbar">
    <div class="heading"><strong>指数顶底区域检查</strong><span>基础 {threshold:.0%} · 分指数波动调整 · 区域顶底 · 日K最高/最低价确认</span></div>
