"""Train and export an immutable all-A ML walk-forward evaluation bundle."""
...
from ..modeling.dataset import DATASET_VERSION, feature_columns
...
def run_pipeline(
    ...
    *,
    ...
    """Verify the stage-one dataset and write test-year OOS predictions."""
    ...
    if dataset_manifest.get("dataset_version") != DATASET_VERSION:
        raise ValueError(f"walk-forward training requires {DATASET_VERSION}")
    ...
    score_weights = SCORE_WEIGHTS_BY_VERSION.get(version)
    if score_weights is None:
        raise ValueError(f"manifest behavior is not frozen for {version}")
    ...
    outputs = {name: path for name, (path, _) in frames.items()}
    ...
    manifest = {
        ...
        "purpose": (
            "Retrospective expanding walk-forward model comparison with only "
            "test-year predictions exported; not untouched forward evidence or "
        ...
        "definition": {
            ...
            "model_parameters": {
                "simple_rule": {
                    "target_fitted_weights": False,
                    "scaling": "training median and interquartile range",
                    "clip": [-3.0, 3.0],
                    "combination": "equal-weight signed mean then sigmoid",
                    "features": {
                        direction: [
                            {"column": column, "sign": sign}
                            for column, sign in definitions
                        ]
                        for direction, definitions in SIMPLE_RULE_FEATURES.items()
                    },
                },
                "elastic_net": {
            ...
            "horizons_trade_days": list(DEFAULT_HORIZONS),
            "calibration": (
                "Validation-only sigmoid calibration; smoothed validation "
                "prevalence fallback for a one-class validation slice."
            ),
            "horizon_projection": "row-wise L2 projection to p5 <= p10 <= p20",
            "score_formula": (
                "100 * ("
                f"{score_weights[0]:g}*p5 + "
                f"{score_weights[1]:g}*p10 + "
                f"{score_weights[2]:g}*p20)"
            ),
            "score_weights": {
                "5d": score_weights[0],
                "10d": score_weights[1],
            ...
            "score_note": (
                "The 20-day probability remains exported and evaluated even "
                "when its composite-score weight is zero."
            ),
            "annual_episode_budget": annual_episode_budget,
            "threshold_selection": (
                "Validation scores only; maximize contiguous episode count at "
                "or below budget, with higher threshold winning ties."
            ),
            "max_alert_active_days": max_alert_active_days,
            "alert_duration_policy": (
                "Keep only the first N trading days of each contiguous raw "
                "threshold run; re-arm only after the raw condition is false."
                if max_alert_active_days is not None
...
def main() -> None:
    ...
    parser.add_argument("--training-version", default=TRAINING_VERSION)
    parser.add_argument(
