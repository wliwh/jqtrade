"""Build the immutable stage-E MA-period breadth decomposition bundle."""
...
from ..signals.definitions.ma_period_breadth_decomposition import (
    CHANGE_LOOKBACK,
    CHANGE_THRESHOLD,
    EXTREME_THRESHOLD,
    MA_WINDOWS,
...
def run_pipeline(
    ...
    *,
    ...
    daily, episodes, comparison = build_ma_period_breadth_signals(
        market,
    ...
    manifest = {
        ...
        "purpose": (
            "Causal MA20/60/120 breadth signals with identical thresholds for "
            "horizon comparison; not a trading strategy."
        ),
        "definition": {
            "ma_windows": list(MA_WINDOWS),
            "signals_per_window": ["top", "bottom"],
            "change_lookback_trade_days": CHANGE_LOOKBACK,
            "top_trigger": "breadth >= 0.70 and five-day change <= -0.05",
            "bottom_trigger": "breadth <= 0.30 and five-day change >= 0.05",
            "extreme_threshold": EXTREME_THRESHOLD,
            "change_threshold": CHANGE_THRESHOLD,
            "raw_value": "the selected MA horizon breadth",
            "valid_count": "the selected MA horizon valid count",
        ...
        "logic": logic_records(
            [
                ...
                / "signals"
                / "definitions"
                / "ma_period_breadth_decomposition.py",
                Path(__file__),
            ]
        ),
        "outputs": [
...
def _validate_input_protocol(manifest: dict[str, object]) -> None:
    if manifest.get("data_version") != INPUT_DATA_VERSION:
        raise ValueError("MA-period decomposition requires all_a_p1_inputs_v2")
    query = manifest.get("query", {})
    if tuple(query.get("ma_windows", [])) != MA_WINDOWS:
        raise ValueError("input MA windows do not match decomposition")
    if (
        query.get("ma_comparison_relative_tolerance") != 1e-12
        or query.get("ma_comparison_absolute_tolerance") != 1e-12
    ):
        raise ValueError("input MA comparison tolerance does not match signals")
