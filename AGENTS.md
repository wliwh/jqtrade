...
- `research/index_turning_points/data/inputs/<数据集>/<版本>/` 是不可变输入快照；JQ 导出程序统一放在 `adapters/jq/`，版本目录保留 manifest、数据和口径说明。不要改写已有 manifest、CSV 或哈希来适配新分析。
- `research/index_turning_points/archive/` 仅保存旧口径代码和实验事实；现役代码如需读取，必须经过显式版本化的 legacy adapter。
