...
<span id="begin_alg"></span>

### 简单但是完整的策略

<span id="full_alg"></span>
...
<span id="alg_introduce"></span> <span id="testing_environment"></span>

### 安全

<span id="safe_introduce"></span>
...
<span id="running_frequency"></span>
...
超额收益<span id="excess_income"></span> 除法版超额收益率说明
...
<span id="sim_trading_caution"></span>
...
<span id="marginsec_close_detail"></span>
...
**返回值** None
**注意** 一个策略中尽量不要同时使用run_daily和handle_data，更不能使用run_daily(handle_data, "xx:xx")
- 建议使用run_daily； 
- [【API解析】策略运行频率（附隔固定时间运行方法）](https://www.joinquant.com/view/community/detail/6797d703ee45325b51079374439b1ca5) - run_daily中的函数只能有一个参数context，具体示例如下：
...
get_extras 获取期货结算价,股票ST状态等

``` python
get_extras(info, security_list, start_date='2015-01-01', end_date='2015-12-31', df=True, count=None)
```

得到多只标的在一段时间的如下额外的数据: - is_st: 是否股改s, st,\*st和退市整理期标的 - futures_sett_price: 期货结算价 - acc_net_value: 基金累计净值 (已废弃,建议直接查询[FUND_NET_VALUE](https://www.joinquant.com/help/api/help#OTCfund:%E8%8E%B7%E5%8F%96%E5%85%AC%E5%8B%9F%E5%9F%BA%E9%87%91%E5%87%80%E5%80%BC%E4%BF%A1%E6%81%AF)表) - unit_net_value: 基金单位净值 (已废弃,建议直接查询[FUND_NET_VALUE](https://www.joinquant.com/help/api/help#OTCfund:%E8%8E%B7%E5%8F%96%E5%85%AC%E5%8B%9F%E5%9F%BA%E9%87%91%E5%87%80%E5%80%BC%E4%BF%A1%E6%81%AF)表) - adj_net_value: 场外基金的复权净值 (已废弃,建议直接查询[FUND_NET_VALUE](https://www.joinquant.com/help/api/help#OTCfund:%E8%8E%B7%E5%8F%96%E5%85%AC%E5%8B%9F%E5%9F%BA%E9%87%91%E5%87%80%E5%80%BC%E4%BF%A1%E6%81%AF)表) - futures_positions: 期货持仓量 (已废弃,建议直接通过get_price获取) **参数** - info: ['is_st', 'acc_net_value', 'unit_net_value', 'futures_sett_price', 'futures_positions', 'adj_net_value'] 中的一个 - security_list: 股票列表 - start_date/end_date: 开始结束日期, 同 [get_price](#get_price) - df: 返回[pandas.DataFrame]对象还是一个dict, 同 [history] - count: 数量, **与 start_date 二选一, 不可同时使用**, 必须大于 0. 表示取 end_date 往前的 count 个交易日的数据 **返回值** - df=True: [pandas.DataFrame]对象, 列索引是股票代号, 行索引是[datetime.datetime], 比如`get_extras('acc_net_value', ['510300.XSHG', '510050.XSHG'], start_date='2015-12-01', end_date='2015-12-03')`返回: \|---\|510300.XSHG\|510050.XSHG\| \|---\|---\|---\| \|2015-12-01\|1.395\|3.119\| \|2015-12-02\|1.4432\|3.251\| \|2015-12-03\|1.4535\|3.254\| `get_extras('is_st', ['000001.XSHE', '000018.XSHE'], start_date='2013-12-01', end_date='2013-12-03')`返回: \|---\|000001.XSHE\|000018.XSHE\| \|---\|---\|---\| \|2013-12-02\|False\|True\| \|2013-12-03\|False\|True\| - df=False 一个dict, key是基金代号, value是[numpy.ndarray], 比如`get_extras('acc_net_value', ['510300.XSHG', '510050.XSHG'], start_date='2015-12-01', end_date='2015-12-03', df=False)` 返回:
...
alpha101 <span id="alpha01"></span> Alpha 101 因子
...
alpha191 <span id="alpha191"></span> Alpha 191 因子
...
technical_analysis <span id="technical_analysis"></span> 技术分析指标
...
<span id="init_subportfolios"></span>set_subportfolios 初始化策略子账户 subportfolios
...
<span id="SubPortfolio"></span>SubPortfolio 子账户信息
...
<span id="transfer_cash"></span>transfer_cash 账户间转移资金
...
margincash_interest_rate <span id="margincash_interest_rate'"></span> 设置融资利率
...
margincash_margin_rate <span id="margincash_margin_rate"></span> 设置融资保证金比率
...
marginsec_interest_rate <span id="marginsec_interest_rate"></span> 设置融券利率
...
marginsec_margin_rate <span id="marginsec_margin_rate"></span> 设置融券保证金比率
...
margincash_open <span id="margincash_open"></span> 融资买入
...
margincash_close <span id="margincash_close"></span> 卖券还款
...
margincash_direct_refund <span id="margincash_direct_refund"></span> 直接还款
...
marginsec_open <span id="marginsec_open"></span> 融券卖出
...
marginsec_close <span id="marginsec_close"></span> 买券还券
...
marginsec_direct_refund <span id="marginsec_direct_refund"></span> 直接还券
...
get_margincash_stocks <span id="get_margincash_stocks"></span> 获取融资标的列表
...
get_marginsec_stocks <span id="get_marginsec_stocks"></span> 获取融券标的列表
...
get_mtss <span id="get_mtss"></span> 获取融资融券信息
...
get_dominant_future <span id="get_dominant_future"></span> 获取主力合约对应的标的
...
get_future_contracts <span id="get_future_contracts"></span> 期货可交易合约列表
...
futures_margin_rate <span id="futures_margin_rate"></span> 设置期货保证金比例
...
is_dangerous <span id="is_dangerous"></span> 期货保证金预警
...
get_price等 <span id="get_future_quotes"></span> 获取期货的行情数据
...
order <span id="order-method_ccfx"></span> 期货按手数下单
...
order_target <span id="order_target_ccfx"></span> 期货目标手数下单
...
order_value <span id="order_value_ccfx"></span> 期货按保证金下单
...
order_target_value <span id="order_target_value_ccfx"></span> 期货目标保证金下单
