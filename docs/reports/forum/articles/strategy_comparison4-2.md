...
这篇文章只介绍这套新框架的设计思路和使用方式，不讨论某个具体策略参数是否最优。下一篇会用 `strategies/etf_rotation/ETF_7star_opt_dynamic.py` 作为实际案例，展示如何通过配置文件在 JQ 平台上完成多轮回测，并分析结果。
...
```yaml
strategy:
  file: "strategies/etf_rotation/ETF_7star_opt_dynamic.py"
