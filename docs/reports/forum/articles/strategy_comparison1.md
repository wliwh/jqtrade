...
> 历史收益曲线图 `default-retLog.png` 未随仓库保存；核心指标保留在下表。
...
> 历史年度收益图 `default-yearR.png` 未随仓库保存；年度数据保留在下表。
...
> 2020 年和 2021 年的历史对比图 `return2020.png`、`return2021-.png` 未随仓库保存；对应差异结论保留如下。
