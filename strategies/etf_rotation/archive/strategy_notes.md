...
> 原文件：`ETFs/ETF_summ.md`。归档于 2026-08-15；其中策略路径、数量和阶段结论不再代表现役目录。
...
6. 七星增强版 (7star / v3.1)
    *   定位：进阶增强版，核心引入了全市场动态扫描与趋势质量过滤。
    *   核心逻辑：**加权对数动量 + R² 过滤**。25 日价格对数线性加权回归，通过 $Slope \times R^2$ 打分。
    *   交易与风控：
        *   混合池机制：固定核心池 (114只) + 成交额 > 5000万的动态行业池。
        *   R² 门槛：要求 $R^2 > 0.4$，筛选“丝滑”趋势。
        *   执行时间：13:10 卖出，13:11 买入 (下午盘确认)。
        *   多维度止损：分钟级监控。支持固定止损 (5%)、当日跌幅止损及 ATR 跟踪止损。
